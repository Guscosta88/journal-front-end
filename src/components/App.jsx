...
const App = () => {
  ...
  //use this to control the effect based on what happens to foo
  ...
  useEffect(() => {
    ...
    const res = await fetch('https://journal-api-production-3368.up.railway.app/categories')
  ...
  useEffect(() => {
    ...
    const res = await fetch('https://journal-api-production-3368.up.railway.app/entries')
    const data = await res.json()
    setEntries(data)
    }
    fetchEntries()
...
const addEntry = async (category, content) => {
  ...
  const returnedEntry = await fetch('https://journal-api-production-3368.up.railway.app/entries', {
    method: 'POST',
    headers: {
